    ...
    try {
      ...
      if (updatedMember) {
        ...
        if (pendingUpdate.loan.change !== 0) {
          if (pendingUpdate.loan.change < 0) {
            // This is a loan payment - use InterestCalculator for proper recalculation
            const paymentAmount = Math.abs(pendingUpdate.loan.change);
            const interestPayment = pendingUpdate.interest.change < 0 ? Math.abs(pendingUpdate.interest.change) : 0;

            const paymentResult = await InterestCalculator.processLoanPayment(
              selectedMember.id,
              paymentAmount,
              interestPayment,
              'admin'
            ...
            if (paymentResult.success) {
              // The InterestCalculator already created the transaction and updated balances
              // Update our local state to reflect the recalculated monthly payment
              const finalUpdatedMember = await db.getMemberById(selectedMember.id);
              if (finalUpdatedMember) {
                setSelectedMember(finalUpdatedMember);
              }
            }
          } else {
