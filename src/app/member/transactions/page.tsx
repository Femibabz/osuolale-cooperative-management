...
export default function TransactionsPage() {
  ...
  useEffect(() => {
    if (user?.role === 'member' && !pageLoaded) {
      try {
        console.log('Loading member data for user:', user.id);
        const memberData = db.getMemberByUserId(user.id);
        console.log('Member data found:', memberData);

        if (memberData) {
          setMember(memberData);
          console.log('Getting transactions for member:', memberData.id);
          const memberTransactions = db.getTransactionsByMember(memberData.id, parseInt(filterPeriod));
          console.log('Transactions found:', memberTransactions);
  ...
  // Update transactions when filter period changes
  useEffect(() => {
    if (member && pageLoaded) {
      try {
        const memberTransactions = db.getTransactionsByMember(member.id, parseInt(filterPeriod));
        setTransactions(memberTransactions);
        setFilteredTransactions(memberTransactions);
      } catch (error) {
  ...
  }, [filterPeriod, member, pageLoaded]);
  ...
  }, [transactions, searchTerm, filterType, pageLoaded]);
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = `${member.memberNumber}_transactions_${new Date().toISOString().split('T')[0]}.csv`;
