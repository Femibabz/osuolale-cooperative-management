...
export default function ApplyLoanPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check loan eligibility using settings
      const eligibilityCheck = LoanCalculator.isEligibleForLoan(member, settings);
      if (!eligibilityCheck.eligible) {
        setError(eligibilityCheck.reason || 'You are not eligible for a loan at this time');
        return;
      }

      // Validate loan amount using settings
      const amountValidation = LoanCalculator.validateLoanAmount(loanAmount, member, settings);
      if (!amountValidation.valid) {
        setError(amountValidation.error || 'Invalid loan amount');
        return;
      }
  ...
  // Calculate loan eligibility using settings
  const totalWithOrganization = member.sharesBalance + member.savingsBalance;
  const maxLoanAmount = LoanCalculator.getMaxLoanAmount(member, settings);
  const eligibilityCheck = LoanCalculator.isEligibleForLoan(member, settings);
  const isEligibleForLoan = eligibilityCheck.eligible;
            ...
            <div className="md:col-span-2">
              <Card className={!isEligibleForLoan ? "opacity-60" : ""}>
                <CardHeader>
                  ...
                  <CardDescription>
                    {isEligibleForLoan
                      ? `Complete the form below to apply for a loan (up to ${settings.loanToSharesSavingsRatio}x your shares + savings)`
                      : eligibilityCheck.reason || "Please clear outstanding balances to apply for a loan"
                    }
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Maximum loan amount: {formatCurrency(maxLoanAmount)} ({settings.loanToSharesSavingsRatio}x your shares + savings)
                      </p>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select onValueChange={(value) => handleSelectChange('duration', value)} disabled={!isEligibleForLoan}>
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value={settings.standardLoanTermMonths.toString()}>{settings.standardLoanTermMonths} months (Standard)</SelectItem>
                ...
                <CardContent className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm font-bold text-blue-600">
                      <span>Maximum Loan Amount ({settings.loanToSharesSavingsRatio}x Total):</span>
